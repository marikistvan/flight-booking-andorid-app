...
exports.askChatGPT = async (req, res) => {
    ...
    if (!prompt || !Array.isArray(prompt) || prompt.length === 0) {
        return res
            .status(400)
            .json({ error: 'Hiányzik a chatHistory vagy üres.' });
    }
            ...
            messages: [
                {
                    role: 'system',
                    content:
                        'Segítőkész repülő asszisztens vagy egy repülőjegy foglaló alkalmazásban. Segítesz utazás és szállás témakörben. Egyéb témákban viszont nem.',
                },
                ...prompt,
            ],
        });
...
exports.sendMessage = async (req, res) => {
    try {
        ...
        if (!chatId || !senderId || !message) {
            return res
                .status(400)
                .json({ error: 'chatId, senderId és message szükséges' });
        }
        ...
        if (chatSnap.exists) {
            ...
            if (
                chatData.participants[0] != senderId &&
                chatData.participants[1] != senderId
            ) {
