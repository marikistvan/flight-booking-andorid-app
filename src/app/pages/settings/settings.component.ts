...
export class SettingsComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (appLang !== null || appLang !== undefined) {
            const language = (
                Object.keys(this.languageDict) as Array<string>
            ).find((key) => this.languageDict[key] === appLang);
            this.actualLanguage.set(language);
        } else {
            const deviceLan = Device.language.split('-')[0];
            const language = (
                Object.keys(this.languageDict) as Array<string>
            ).find((key) => this.languageDict[key] === deviceLan);
            this.actualLanguage.set(language ?? '');
        }
        ...
        notification === 'True'
            ? this.isNotificationEnabled.set(true)
            : this.isNotificationEnabled.set(false);
    }
    ...
    setAppLanguage() {
        action({
            ...
            if (result !== localize('general.cancel')) {
                const localeOverriddenSuccessfully = overrideLocale(
                    this.languageDict[result]
                );
    ...
    async chooseDestination(): Promise<void> {
        ...
        if (result as LocationResponse) {
            const defaultDes = this.formatName(
                result.detailedName,
                result.iataCode
            );
    ...
    requestPermission() {
        LocalNotifications.requestPermission().then((granted) => {
            if (
                granted &&
                getString('isNotificationEnabled', 'True') === 'True'
            ) {
    ...
    scheduleMonthlyPromo() {
        ...
        LocalNotifications.schedule([
            ...
            (scheduledIds) =>
                console.log('Havi promóció ütemezve, ID:', scheduledIds),
