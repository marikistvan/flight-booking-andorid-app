...
export class SelectSeatComponent implements OnInit, AfterViewInit {
    ...
    @ViewChild('planeContainer', { static: false })
    planeContainerRef!: ElementRef;
    ...
    ngAfterViewInit(): void {
        ...
        for (
            let i = 0;
            i <= this.seats.decks[0].deckConfiguration.length;
            i++
        ) {
            this.seatsGridLayout.addRow(new ItemSpec(1, 'auto'));
        }
        this.seatsGridLayout.className =
            'select-seat-component-seats-gridlayout';
       ...
       this.seatsGridLayout.addChild(this.label(this.placeColumnName[5], 'select-seat-component-place-col-name', 6, 0));*/
        ...
        for (
            let i = 0;
            i < this.seats.decks[0].deckConfiguration.endSeatRow;
            i++
        ) {
            this.seatsGridLayout.addChild(
                this.label(
                    (i + 1).toString(),
                    'select-seat-component-none-seat-label',
                    3,
                    i + 1
                )
            );
        ...
        this.seats.decks[0].seats.forEach((element) => {
            if (
                element.travelerPricing[0].seatAvailabilityStatus ===
                'AVAILABLE'
            ) {
                this.seatsGridLayout.addChild(
                    this.label(
                        '',
                        'select-seat-component-available-seat-label',
                ...
                element.travelerPricing[0].seatAvailabilityStatus === 'BLOCKED'
            ) {
                this.seatsGridLayout.addChild(
                    this.label(
                        '',
                        'select-seat-component-blocked-seat-label',
                        element.coordinates.y,
                        element.coordinates.x,
                        element.number
                    )
                ...
                this.seatsGridLayout.addChild(
                    this.label(
                        '',
                        'select-seat-component-underReservation-seat-label',
                        element.coordinates.y,
                        element.coordinates.x,
                        element.number
                    )
                ...
                this.seatsGridLayout.addChild(
                    this.label(
                        '',
                        'select-seat-component-OCCUPIED-seat-label',
                        element.coordinates.y,
                        element.coordinates.x,
                        element.number
                    )
        ...
        if (this.seats.decks[0]?.facilities !== undefined) {
            this.seats.decks[0].facilities.forEach((element) => {
                if (element.code === 'LA') {
                    this.seatsGridLayout.addChild(
                        this.image(
                            '~/assets/icons/lavatory.png',
                            'select-seat-component-icons-img',
                            element.coordinates.y,
                            element.coordinates.x
                        )
                    );
                    ...
                    this.seatsGridLayout.addChild(
                        this.label(
                            element.column,
                            'select-seat-component-place-col-name',
                            element.coordinates.y,
                            element.coordinates.x
                        )
                    );
    ...
    testF(seatNumber: string, lbl: Label) {
        ...
        const elementStatus = this.seats.decks[0].seats.find(
            (element) => element.number === seatNumber
        );
        let total_price =
            elementStatus.travelerPricing[0]?.price?.total ?? '-1';
        ...
        this.modalDialogService
            .showModal(SeatDetailsModalComponent, options)
            .then((result) => {
