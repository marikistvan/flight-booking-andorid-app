...
export class BasketComponent implements OnInit {
    ...
    getFlightsFromDatabase(): void {
        this.basicFlightInformations =
            new ObservableArray<BasicFlightInformation>();
        if (this.user != null) {
            firebase()
                .firestore()
                ...
                .then((querySnapshot) => {
                    querySnapshot.forEach((doc) => {
                        ...
                        this.basicFlightInformations.push({
                            data_itineraries_segments_departure_at:
                                data.departureAt,
                            data_itineraries_segments_arrival_at:
                                data.arrivalAt,
                            data_itineraries_segments_departure_iataCode:
                                data.departureIata,
                            data_itineraries_segments_arrival_iataCode:
                                data.arrivalIata,
                    ...
                    for (
                        let i = 0;
                        i < this.basicFlightInformations.length;
                        i++
                    ) {
    ...
    Delete(id: string): void {
        ...
        firebase()
            ...
            .collection('flights')
            .doc(
                this.basicFlightInformations.getItem(parseInt(index)).data_id +
                    '_' +
                    this.basicFlightInformations.getItem(parseInt(index))
                        .data_itineraries_duration
            )
            .delete()
            .then(() => {
